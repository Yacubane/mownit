...
void test_vector() {
    ...
    while(size <= MAX_VECTOR_SIZE) {
        for(int iteration = 0; iteration < 10; iteration++) {
            unsigned long long int result = ddot_test(double_vector, double_vector, size, &scalar_result);
...
void test_matrix() {
    ...
    while(size <= MAX_MATRIX_SIZE) {
        for(int iteration = 0; iteration < 10; iteration++) {
            unsigned long long int result = dgemv_test(double_matrix, double_vector, size, double_result_vector);
            snprintf(buffer, 255, "%d,%d,%lf\n", size, iteration, result);
            int res = fwrite(buffer, 1, strlen(buffer), csv);
            printf("%s\n", buffer);
        }
...
void test_all() {
    ...
    while(size <= MAX_MATRIX_SIZE) {
        for(int iteration = 0; iteration < 10; iteration++) {
            unsigned long long int result1 = ddot_test(double_vector, double_vector, size*size, &scalar_result);
            unsigned long long int result2 = dgemv_test(double_matrix, double_vector, size, double_result_vector);
            snprintf(buffer, 255, "%d,%d,%llu,%llu\n", size*size, iteration, result1, result2);
            int res = fwrite(buffer, 1, strlen(buffer), csv);
            printf("%s\n", buffer);
        }
        size += 100;
...
void test_all_small() {
    ...
    FILE* csv = fopen("all_test_small.csv", "w");
    char *buffer = malloc(255 * sizeof(char));
    snprintf(buffer, 255, "size,iteration,vector_time,matrix_time\n");
    fwrite(buffer, 1, strlen(buffer), csv);
    int size = 1;
    ...
    while(size <= maxSize) {
        for(int iteration = 0; iteration < 10; iteration++) {
            unsigned long long result1 = ddot_test(double_vector, double_vector, size, &scalar_result);
            unsigned long long result2 = dgemv_test(double_matrix, double_vector, size, double_result_vector);
